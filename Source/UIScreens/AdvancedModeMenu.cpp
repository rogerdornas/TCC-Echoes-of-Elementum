    ...
    :UIScreen(game, fontName, isClosable)
{
    ...
    text = AddText(textMessage, Vector2::Zero,  34);
    text->SetPosition(Vector2(960, 600));

    textMessage = "DESEJA JOGAR ESSE MODO?";

    text = AddText(textMessage, Vector2::Zero,  34);
    text->SetPosition(Vector2(960, 710));

    AddButton("SIM", Vector2(820, 770), Vector2::Zero, 34, UIButton::TextPos::Center,
    [this]() {
    ...
    AddButton("NÃO", Vector2(1020, 770), Vector2::Zero, 34, UIButton::TextPos::Center,
    [this]() {
        Close();
    }
    , true);
